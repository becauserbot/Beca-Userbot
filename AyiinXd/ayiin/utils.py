    ...
    try:
        r = await bot(
            CreateChannelRequest(
                title="ʙᴇᴄᴀ ᴜsᴇʀʙᴏᴛ Lᴏɢs",
                about="» Group log Created by: ʙᴇᴄᴀ ᴜsᴇʀʙᴏᴛ\n\n» Support : ʙᴇᴄᴀ \n» Support: ʙᴇᴄᴀ ",
                megagroup=True,
            ),
        )
...
async def autobot():
    ...
    if who.username:
        ...
        username = f"Beca{(str(who.id))[5:]}bot"
    bf = "@BotFather"
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = f"Beca{str(who.id)[6:]}{ran}bot"
        await bot.send_message(bf, username)
        await asyncio.sleep(1)
